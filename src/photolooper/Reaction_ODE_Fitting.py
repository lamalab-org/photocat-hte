...
def ODE_matrix_fit_func(k, initial_state, t, matrix, ravel = True, 
			flux = np.array([1]), cross_section = np.array([1]), idx = None):
...
def ODE_fitting(data, t, reaction_string, idx = None, upper_bound = 1., bounds_arr = None, workers = 1):
    ...
    p = differential_evolution(func=residual_ODE, bounds = bounds_arr, args = (initial_state, t, data, matrix, idx), workers = workers)
    p_ode = p.x
    residual = p.fun
